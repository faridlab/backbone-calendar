...
metaphor migration generate Invoice calendar
metaphor migration run
...
(`calendar` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed calendar          # run Rust seeders
metaphor migration generate-seeds calendar  # emit SQL seed files
```
