//! Domain policy for CalendarEventSeries aggregate
//!
...
/// Domain policy for CalendarEventSeries — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<CalendarEventSeries>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CalendarEventSeriesDomainPolicy = PermitAllPolicy<CalendarEventSeries>;
