//! Domain policy for CalendarEvent aggregate
//!
...
/// Domain policy for CalendarEvent — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<CalendarEvent>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CalendarEventDomainPolicy = PermitAllPolicy<CalendarEvent>;
