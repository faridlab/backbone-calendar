//! Domain policy for CalendarEventAttendee aggregate
//!
...
/// Domain policy for CalendarEventAttendee — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<CalendarEventAttendee>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type CalendarEventAttendeeDomainPolicy = PermitAllPolicy<CalendarEventAttendee>;
